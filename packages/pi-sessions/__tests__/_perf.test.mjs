...
test('clearSessionsCache: the explicit hook makes a new session visible immediately', async () => {
  ...
  assert.ok(
    first.some((s) => s.cwd === '/fresh-a'),
    'first session listed'
  );
  ...
  assert.equal(
    stale.some((s) => s.cwd === '/fresh-b'),
    false,
    'cached serve does not scan'
  );
  ...
  assert.ok(
    fresh.some((s) => s.cwd === '/fresh-b'),
    'explicit invalidation reveals the new session'
  );
});
...
test('session listing: a huge session file is scanned with a line cap', async () => {
  freshAgentDir();
  writeSession('projHuge', 'big.jsonl', [{ type: 'session', cwd: '/huge', timestamp: '2026-07-01T00:00:00Z' }]);
