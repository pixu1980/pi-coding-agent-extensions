...
export async function showSessionSidebar(ctx: ExtensionCommandContext): Promise<void> {
  ...
  const result = await ctx.ui.custom<SessionSummary | undefined>((tui, theme, _keybindings, done) => {
...
export async function showFolderSidebar(ctx: ExtensionCommandContext): Promise<void> {
  ...
  while (navDepth < MAX_NAV_DEPTH) {
    ...
    const folderResult = await ctx.ui.custom<FolderSummary | undefined>((tui, theme, _keybindings, done) => {
      const modal = new SessionCatalogModalComponent(
        theme,
    ...
    const sessionResult = await ctx.ui.custom<SessionSummary | undefined>((tui, theme, _keybindings, done) => {
      const modal = new SessionCatalogModalComponent(
        theme,
        (selection) => done(selection as SessionSummary | undefined),
        tui.terminal.rows,
      ...
      modal.showSessions(folderResult.sessions, `📁 ${folderResult.folder.replace(homedir(), '~')}`);
 ...
 */
async function loadSession(ctx: ExtensionCommandContext, sessionFile: string): Promise<void> {
  try {
    ...
    ctx.ui.notify(`Could not switch directly. Try /resume to select the session.`, 'warning');
  }
}
