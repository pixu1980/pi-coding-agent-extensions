...
function parseEndpoint(value: string | URL | undefined): URL | undefined {
	...
	if (value instanceof URL) return value.protocol === "https:" && isAllowedCursorHost(value.hostname) ? value : undefined;
...
export function checkBackendOverride(env: EgressEnv = process.env): BackendOverrideCheck {
	...
	if (isAllowedCursorEndpoint(override)) return { ok: true, override };
...
export function formatEgressSurface(surface: EgressSurface = describeEgressSurface()): string {
	...
	if (!surface.override.ok) lines.push(`- BLOCKED: ${surface.override.reason}`);
	else if (surface.override.reason) lines.push(`- note: ${surface.override.reason}`);
	return lines.join("\n");
}
