 ...
 * is about behavior we control: what reaches the transcript, what reaches the
...
describe('describeCursorToolCall', () => {
  it('summarizes shell, read and grep calls without content', () => {
    assert.equal(
...
describe('routeCursorDelta', () => {
  ...
  it('turns tool activity into trace lines', () => {
    ...
    routeCursorDelta(
      { type: 'tool-call-started', callId: '1', toolCall: { type: 'shell', args: { command: 'ls' } } },
      s
    );
    routeCursorDelta(
      { type: 'tool-call-completed', callId: '1', toolCall: { type: 'shell', args: { command: 'ls' } } },
      s
    );
...
describe('planCursorTurn', () => {
  ...
  it('defaults the thinking level from the model capability', () => {
    assert.equal(
      planCursorTurn(makeModel(), makeContext(), { hasLiveAgent: true }).selection.params[0].value,
      'medium'
    );
    assert.equal(
...
describe('streamCursor', () => {
  ...
  it('skips the SDK entirely for an empty prompt', async () => {
    const sdk = fakeSdk([]);
    const { message } = await collect(streamCursor(makeModel(), { messages: [] }, { apiKey: KEY }, deps(sdk)));
        ...
        create: async () => ({
          ...
          send: async () => ({
            ...
            wait: async () => {
              ...
              // The Cursor SDK reports this exact spelling (RunStatus).
