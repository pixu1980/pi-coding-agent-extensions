...
//
// Every emoji-labelled string the extension renders must use the optical
...
test("autocomplete: /effort + space shows the same menu as /reasoning", async () => {
  ...
  for (const item of viaEffort.items) {
    if (item.value === "auto") assertCanonicalLabel(item.label);
    else assertLevelLabel(item.label, item.value);
  }
});
