...
function encodeModelId(modelId: string, context: string | undefined, fastOverride: boolean | undefined): string {
	...
	if (fastOverride === true) return `${withContext}:fast`;
	if (fastOverride === false) return `${withContext}:slow`;
	return withContext;
}
	...
	for (const item of sorted) {
		...
		for (const context of contexts) {
			for (const fastOverride of fastOverrides) {
				...
				if (seen.has(piModelId)) continue;
				seen.add(piModelId);
				identities.push({
					item,
					piModelId,
...
function upsertParam(params: { id: string; value: string }[], id: string, value: string): { id: string; value: string }[] {
	...
	if (!next.some((param) => param.id === id)) next.push({ id, value });
	return next;
}
...
function applyThinkingLevel(
	...
	if (mapped === undefined || mapped === null) return params;

	const { reasoning, effort, thinking } = metadata.parameterIds;

	if (level === "off") {
		if (reasoning) return upsertParam(params, "reasoning", mapped);
		if (thinking) return upsertParam(params, "thinking", mapped);
		return params;
	}
...
export function buildModelSelection(
	...
	if (!metadata) return { id: piModelId };
 ...
 * live catalog replaces it as soon as a key exists.
 */
...
async function runDiscovery(options: DiscoverCatalogOptions, offline: boolean): Promise<DiscoverCatalogResult> {
	...
	try {
		...
		return {
			metadata: registerCatalog(FALLBACK_CURSOR_MODELS),
			source: "fallback",
			note: `Cursor model discovery failed (${error instanceof Error ? error.name : "error"}); using the placeholder catalog.`,
		};
	}
}
