...
> This package is a fork of pi-mcp-adapter with one key difference: the direct-tools advisory threshold is configurable via `settings.directToolsAdvisoryThreshold` and defaults to **500** instead of 75. All credit for the original design and implementation goes to Nico.
...
```bash
pi install npm:@pixu1980/pi-mcp
```
...
| Setting | Description |
|---------|-------------|
| `toolPrefix` | `"server"` (default), `"short"` (strips `-mcp` suffix), `"none"` (bare dash-separated tool commands, bare prompt commands), or `"mcp"` (prefixes with `mcp__`, using server-mode normalization). Per-server `toolPrefix` overrides this for that server. |
