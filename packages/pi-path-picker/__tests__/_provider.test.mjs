...
test("natural typing (no Tab) inside quotes delegates to native", async () => {
  for (const path of ["/", "~/", "./", "../"]) {
    ...
    assert.equal(calls.filter(([name]) => name === "getSuggestions").length, 1, `${path} without Tab must delegate once`);
  }
});
...
test("apostrophe inside double quotes is plain text, not a broken pair", async () => {
  ...
  assert.equal(suggestions.items.some((item) => item.value === "./it's.txt" || item.value === "./it's.txt/"), true, "should complete the apostrophe file");
  assert.equal(calls.filter(([name]) => name === "getSuggestions").length, 0);
});
...
test("paths with spaces are captured inside the quotes", async () => {
  ...
  assert.equal(suggestions.items.some((item) => item.value === "./My Projects/notes.md"), true, "should complete inside the spaced directory");
  assert.equal(calls.filter(([name]) => name === "getSuggestions").length, 0);
});
...
test("applyCompletion keeps the closing quote of a closed pair", async () => {
  ...
  const applied = provider.applyCompletion(['"./"'], 0, 4, suggestions.items.find((item) => item.value === "./alpha.txt"), suggestions.prefix);
...
test("applyCompletion for a partial name keeps the directory prefix", async () => {
  ...
  const applied = provider.applyCompletion(['"./s'], 0, 4, suggestions.items.find((item) => item.value === "./src/"), suggestions.prefix);
...
test("first Tab lists EVERY file and directory (no cap)", async () => {
  ...
  assert.equal(first.items.some((item) => item.value === "./f40.txt"), true, "items beyond the old cap must be visible");
});
...
test("first Tab already includes hidden files; repeated Tab is stable", async () => {
  ...
  assert.equal(first.items.some((item) => item.value === "./.env"), true, "hidden files must be listed on the first Tab");
  assert.equal(first.items.some((item) => item.value === "./subdir/"), true, "directories must be listed");
...
test("hidden entries are listed without any dot prefix", async () => {
  ...
  assert.equal(suggestions.items.some((item) => item.value === "./.hidden.txt"), true, "dotfiles must be in the complete listing");
});
...
test("names with control characters (macOS Icon\\r) are skipped", async () => {
  ...
  assert.equal(complete.items.some((item) => item.value.includes("Icon")), false, "Icon\\r must not appear in the complete listing");
  assert.equal(complete.items.some((item) => item.value === "./normal.txt"), true, "regular files must stay");
  assert.ok(complete.items.every((item) => !/[\u0000-\u001F\u007F]/.test(item.value)), "no item value may contain control characters");

  const filtered = await provider.getSuggestions(['"./n'], 0, 4, { ...SIG, force: true });

  assert.equal(filtered.items.some((item) => item.value === "./normal.txt"), true, "filtered listing must keep working");
  assert.equal(filtered.items.some((item) => item.value.includes("Icon")), false, "Icon\\r must not slip into filtered listings either");
});
...
test("a directory token lists complete contents; a partial name filters", async () => {
  ...
  assert.equal(root.items.some((item) => item.value === "./.env"), true, "dotfiles belong to the complete listing");
  ...
  assert.equal(partial.items.some((item) => item.value === "./f40.txt"), true, "the last f* file must still be visible (no cap)");
  assert.equal(partial.items.some((item) => item.value === "./.env" || item.value === "./subdir/"), false, "non-matching dotfiles and dirs must not appear");
  ...
  assert.equal(deep.items.some((item) => item.value === "./subdir/inner.txt"), true);
  ...
  assert.equal(inner.items.some((item) => item.value === "./subdir/inner.txt"), true);
});
...
test("dot-prefix partials reveal hidden entries", async () => {
  ...
  assert.equal(suggestions.items.some((item) => item.value === "./.hidden.txt"), true, "a .-prefixed query must match hidden files");
  assert.equal(suggestions.items.some((item) => item.value === "./.other.txt"), false, "dotfiles outside the prefix must stay hidden");
});
