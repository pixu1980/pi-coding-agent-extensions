...
export class McpOAuthProvider implements OAuthClientProvider {
  ...
  async clientInformation(): Promise<OAuthClientInformationMixed | undefined> {
    ...
    if (this.config.clientId) {
      ...
      const clientSecret = this.config.clientSecret?.startsWith('!')
        ? await resolveCommandSecret(this.config.clientSecret, `MCP server "${this.serverName}" OAuth clientSecret`)
  ...
  async saveClientInformation(info: OAuthClientInformationMixed): Promise<void> {
    ...
    const redirectUris =
      ('redirect_uris' in info ? info.redirect_uris : undefined) ?? (this.redirectUrl ? [this.redirectUrl] : undefined);
