...
import { McpOAuthProvider, type McpOAuthConfig } from './_mcp-oauth-provider.ts';
import {
...
import { isServerDisabled, type ServerEntry } from './_types.ts';
...
export async function startAuth(
  ...
  try {
    ...
    await setPendingAuth(
      runtime,
      serverName,
      { serverName, authProvider, serverUrl, authorizationUrl: capturedUrl.toString(), discovery, authStorageOptions },
      oauthState,
      signal,
      generation
    );
