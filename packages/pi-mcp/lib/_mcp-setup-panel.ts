...
export class McpSetupPanel {
  ...
  render(width: number): string[] {
    ...
    if (this.notice) {
      const tone =
        this.notice.tone === 'success' ? this.t.success : this.notice.tone === 'warning' ? this.t.warning : this.t.hint;
  ...
  private secondarySummaryLine(): string {
    ...
    const conflictNote =
      this.discovery.conflicts.length > 0
        ...
        : '';
    const importIssueNote =
      this.discovery.importIssues.length > 0
      ...
      case 'adopt-imports':
        return this.formatWritePreview(
          ...
          this.callbacks.previewImports(
            this.discovery.imports.filter((entry) => this.selectedImports.has(entry.kind)).map((entry) => entry.kind)
          ),
          [
