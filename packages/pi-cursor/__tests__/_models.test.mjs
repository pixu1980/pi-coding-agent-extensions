...
describe("parseContextWindow", () => {
	...
	it("returns undefined for labels it cannot vouch for", () => {
		for (const value of ["default", "", "lots", "200"]) assert.equal(parseContextWindow(value), undefined, value);
	});
});
...
describe("buildModelIdentities", () => {
	...
	it("expands speed only when the model exposes a fast parameter", () => {
		const ids = buildModelIdentities([{ id: "m", displayName: "M", parameters: [{ id: "fast", values: [{ value: "true" }] }] }]).map(
			(identity) => identity.piModelId,
		);
...
describe("discoverCursorCatalog", () => {
	...
	it("returns the static fallback without a key, with a note", async () => {
		...
		assert.deepEqual(
			...
			registerCatalog(FALLBACK_CURSOR_MODELS).map((entry) => entry.piModelId).sort(),
		);
	});
