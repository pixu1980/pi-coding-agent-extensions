...
export async function resolveCursorApiKey(options: ResolveApiKeyOptions = {}): Promise<string | undefined> {
	...
	if (options.allowStored === false) return undefined;
...
export async function describeApiKeySource(
	...
	if (normalizeApiKey(options.explicit)) return "request";
	if (readEnvApiKey(options.env ?? process.env)) return "environment";
	if (options.allowStored === false) return "none";
