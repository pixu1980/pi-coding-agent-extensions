 ...
 *
 * The tool trace is deliberately bounded and summarized (command, path, pattern)
...
export class CursorContentEmitter {
  ...
  appendActivity(line: string): void {
    if (this.activityTruncated || !line) {
      return;
    }
...
/**
 * Summarize a Cursor tool call into one trace line.
 *
 ...
 * time and an unrecognized delta must never break a turn.
 */
...
export function streamCursor(
  ...
  void (async () => {
    ...
    try {
      ...
      if (!overrideCheck.ok) {
        throw new Error(overrideCheck.reason);
      }
