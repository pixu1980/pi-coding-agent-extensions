...
test('AskParams is a TypeBox object schema with the expected properties', () => {
  ...
  assert.equal(
    AskParams.properties?.allowOther?.anyOf?.[1]?.default ?? AskParams.properties?.allowOther?.default,
    undefined
  );
});
...
test('normalizeQuestions honors a start index for labels', () => {
...
test('normalizeInterview keeps labeled waves and stamps each question', () => {
  const out = normalizeInterview({
    waves: [
      {
...
test('normalizeInterview defaults question labels per wave', () => {
  ...
  assert.deepEqual(
    out.waves[0].questions.map((q) => q.label),
    ['Q1']
  );
  assert.deepEqual(
    out.waves[1].questions.map((q) => q.label),
    ['Q1']
  ); // labels restart per wave
});
...
test('formatInterviewLine prefixes the question label', () => {
  const out = formatInterviewLine('Scope', [{ value: 'fe', label: 'Frontend', index: 2, note: 'team prefers it' }]);
