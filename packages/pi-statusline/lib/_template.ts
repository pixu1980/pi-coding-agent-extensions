...
function resolveToken(token: string, data: StatusLineData): string {
  switch (token) {
    ...
    case 'git_status':
      return fmtGit(data);
    case 'git_ahead':
      return data.git && data.git.ahead > 0 ? String(data.git.ahead) : '';
    case 'git_behind':
      return data.git && data.git.behind > 0 ? String(data.git.behind) : '';
    case 'git_dirty':
      return data.git && data.git.dirty > 0 ? String(data.git.dirty) : '';
    case 'model':
      return data.model;
    case 'effort':
      return data.effort;
    case 'context':
      return `${fmtTokens(data.contextUsed)}/${fmtTokens(data.contextTotal)} (${data.contextPct}%)`;
    case 'context_used':
      return fmtTokens(data.contextUsed);
    case 'context_total':
      return fmtTokens(data.contextTotal);
    case 'context_pct':
      return `${data.contextPct}%`;
    case 'initial_prompt':
      return data.initialPrompt;
    default:
      return `{${token}}`;
  }
}
  ...
  for (let i = 0; i < segs.length; i++) {
    ...
    if (s.type === 'static') {
      ...
      if (next && next.type === 'token' && next.optional) {
        ...
        const prev = segs[i - 1];
        const isCtxSep =
          prev?.type === 'token' && next?.type === 'token' && CTX_TOKEN.has(prev.name) && CTX_TOKEN.has(next.name);
