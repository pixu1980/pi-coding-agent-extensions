...
import type { ToolMetadata, McpContent } from "./_types.ts";
import { getServerPrefix, isServerDisabled, parseUiPromptHandoff } from "./_types.ts";
import { lazyConnect, markKeepAliveAfterConnect, notifyToolMetadataUpdated, updateServerMetadata, updateMetadataCache, getFailureAgeSeconds, updateStatusBar, clearFailure, recordFailure } from "./_init.ts";
import { abortable, throwIfAborted } from "./_abort.ts";
import { combineAbortSignals, isAbortError } from "./_runtime-owner.ts";
import { buildToolMetadata, getToolNames, findToolByName, formatSchema } from "./_tool-metadata.ts";
import { reconstructPromptMetadata } from "./_metadata-cache.ts";
...
import { formatAuthRequiredMessage, formatMcpStatus, resolveServerUrl, truncateAtWord } from "./_utils.ts";
import { authenticate, completeAuthFromInput, startAuth, supportsOAuth } from "./_mcp-auth-flow.ts";
...
export function executeSearch(
  ...
  try {
    if (regex) {
      ...
      const escaped = terms.map((t) => t.replaceAll(/[.*+?^${}()|[\]\\]/g, "\\$&"));
