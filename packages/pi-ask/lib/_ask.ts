 ...
 *
 * `pi` is optional. When supplied it reaches pi-path-picker over the event bus, so
...
export function createAskTool(pi?: PathProviderBus): ToolDefinition<typeof AskParams, AskDetails> {
  return {
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const result = await ctx.ui.custom<{ answers: SelectAnswer[]; canceled: boolean }>((tui, theme, _kb, done) => {
        ...
        function submit(answers: SelectAnswer[]) {
          done({ answers: answers.map((a) => withNote(a, state.noteText)), canceled: false });
        }
        ...
        function submitAll() {
          ...
          for (const i of ordered) {
            ...
            if (opt && !opt.isOther) {
              answers.push(selectionFromIndex(displayOptions, i));
            }
          }

          if (state.customEntry) {
            answers.push(state.customEntry);
          }
        ...
        function render(width: number): string[] {
          ...
          if (mustRebuildForAutocomplete(editor)) {
            cachedLines = undefined;
          }
