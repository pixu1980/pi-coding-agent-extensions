...
/** A labeled group of questions (e.g. "Wave 1 - Baseline"). */
...
export interface AskDetails {
  ...
  options: string[];
  /** Single-select answer; null when canceled or when multiSelect is used */
  answer: SelectAnswer | null;
  /** Multi-select answers; null when single-select or canceled */
 ...
 * `waves` array (labeled groups) or a flat `questions` list, which is
  ...
  if (params.waves && params.waves.length > 0) {
    for (const wave of params.waves) {
      ...
      for (const q of normalized) {
        q.waveLabel = wave.label?.trim() || undefined;
      }
