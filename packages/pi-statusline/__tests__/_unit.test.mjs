...
writeFileSync(
  ...
  JSON.stringify({ mcpServers: { server1: { command: 'npx' } } })
);
...
test('resolveTemplate: custom format uses custom template, falls back to preset-compact', () => {
  assert.equal(resolveTemplate({ ...DEFAULT_SETTINGS, format: 'custom', customTemplate: '{model}' }), '{model}');
  assert.equal(
...
test("renderResponsive: compact/minimal keep the '/' between used and total gradient-colored", () => {
  ...
  for (const line of [compact, minimal]) {
    ...
    assert.ok(
      !line.includes('\x1b[38;2;140;140;140m/\x1b[0m') && !line.includes('901k\x1b[0m\x1b[38;2;140;140;140m/'),
