/**
 * pi-ask — `questionnaire` tool (multi-question interview)
 *
 * Tab-bar navigation between questions, a review/Submit tab, per-question
 * notes (`n`), custom answers ("Type something.") and multi-select.
 *
 * Left/right arrows (or Tab) switch tabs freely — the user can hop between
 * questions at will, answering each one with digits (record + advance) or
 * by highlighting an option with ↑/↓. Enter on a question tab records the
 * highlighted option (or the current multi-selects) and jumps straight to
 * the review tab; Enter on the review tab submits everything. A note is
 * armed with `n` before selecting and travels with the next answer.
 */
...
/**
 * The `questionnaire` tool definition registered by the extension.
 */
export function createQuestionnaireTool(): ToolDefinition<typeof QuestionnaireParams, QuestionnaireDetails> {
	return {
		name: "questionnaire",
		label: "Questionnaire",
		description:
			"Ask the user a structured set of questions (interview). Each question supports options, custom answers and notes; a review tab shows all answers before submission. Use for requirements gathering, preference discovery, or decision confirmation.",
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			if (ctx.mode !== "tui") {
				return {
					...
					details: { title, questions, answers: [], cancelled: true } as QuestionnaireDetails,
				};
			}
			...
			const result = await ctx.ui.custom<QuestionnaireUIResult>((tui, theme, _kb, done) => {
				...
				for (const q of questions) sessions.set(q.id, buildSession(q));
				...
				function recorded(): RecordedAnswer[] {
					...
					for (const q of questions) {
						...
						out.push({ questionId: q.id, questionLabel: q.label, answers: answersFor(s) });
					}
					return out;
				}
				...
				/** Record the answers and move to the next tab (digit quick-flow). */
				function recordAndAdvance(s: QuestionSession, answers: SelectAnswer[]) {
					applyAnswers(s, answers);
					advance();
				}

				/** Record the answers and jump straight to the review tab (Enter flow). */
				function recordAndGoReview(s: QuestionSession, answers: SelectAnswer[]) {
					applyAnswers(s, answers);
					currentTab = order.length; // review tab
					refresh();
				}
				...
				editor.onSubmit = (value) => {
					...
					if (editMode && s) {
						...
						editMode = false;
						editor.setText("");
						recordAndGoReview(s, [withNote(custom, s.noteText)]);
					} else if (noteMode && s) {
				...
				function handleInput(data: string) {
					...
					if (currentTab === order.length) {
						if (matchesKey(data, Key.enter) && recorded().length === questions.length) {
					...
					if (matchesKey(data, Key.enter)) {
						if (s.q.multiSelect) {
							if (s.multi.size > 0 || s.customEntry) {
								recordAndGoReview(s, answersFor(s));
							}
						} else if (s.opts[s.optionIndex]?.isOther) {
							moveCursor(s, s.optionIndex); // ensure write mode
						} else {
							recordAndGoReview(s, [withNote(selectionFromIndex(s.opts, s.optionIndex), s.noteText)]);
						}
						return;
					}
				...
				function render(width: number): string[] {
					...
					for (let i = 0; i < questions.length; i++) {
						...
						const answered = isAnswered(sessions.get(questions[i].id)!);
						const box = answered ? "■" : "□";
					...
					const canSubmit = recorded().length === questions.length;
					...
					if (editMode) {
						...
						lines.push("");
						const progress = `${currentTab + 1}/${questions.length}`;
						if (s.q.multiSelect) {
							addWrappedWithPrefix(" ", theme.fg("dim", `${progress} • ←→ tabs • ↑↓/1-9 move • Space toggle • Enter review • n note • Esc back`));
						} else {
							addWrappedWithPrefix(" ", theme.fg("dim", `${progress} • ←→ tabs • ↑↓ move • 1-9 answer • Enter review${s.q.allowNote ? " • n note" : ""} • Esc back`));
						}
					}
			...
			const content = result.answers.map((r) => formatQuestionnaireLine(r.questionLabel, r.answers)).join("\n");
		...
		renderResult(result, _options, theme, _context) {
			...
			const lines = details.answers.map((a) => `${theme.fg("success", "✓ ")}${theme.fg("accent", a.questionLabel)}: ${summarizeAnswers(a.answers)}`);
