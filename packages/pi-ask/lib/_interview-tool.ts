    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      for (const chunk of chunks) {
        const result = await ctx.ui.custom<ChunkUIResult>((tui, theme, _kb, done) => {
          ...
          for (const q of flat) {
            sessions.set(q.id, buildSession(q));
          }
          ...
          // Path completion for custom answers and notes; a no-op when
          ...
          function answersFor(s: QuestionSession): SelectAnswer[] {
            ...
            for (const i of ordered) {
              ...
              if (opt && !opt.isOther) {
                out.push(selectionFromIndex(s.opts, i));
              }
            }

            if (s.customEntry) {
              out.push(s.customEntry);
            }

            if (out.length === 0 && s.answer) {
              out.push(s.answer);
            }
          ...
          function isAnswered(s: QuestionSession): boolean {
            if (s.q.multiSelect) {
              return s.multi.size > 0 || s.customEntry !== null;
            }
          ...
          function handleInput(data: string) {
            ...
            if (currentTab === order.length) {
              if (matchesKey(data, Key.enter) && recorded().length === flat.length) {
                done({ answers: recorded(), canceled: false });
            ...
            if (matchesKey(data, Key.escape)) {
              if (currentTab > 0) {
                ...
                done({ answers: recorded(), canceled: true });
              }
            }
          }
